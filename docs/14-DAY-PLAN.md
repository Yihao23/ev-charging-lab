...
- [X] Read [`GLOSSARY.md`](GLOSSARY.md) top to bottom. Then close it and write
  the ISO 15118 stack from memory, bottom to top. Check yourself.
  读完术语表，合上，默写 ISO 15118 协议栈，然后对答案。
- [X] `pip install ocpp websockets`, run the upstream `v16` example from
  [mobilityhouse/ocpp](https://github.com/mobilityhouse/ocpp).
- [X] Answer in one sentence each: Why PLC and not WiFi? Why EXI and not XML?
...
- [X] Run `csms.central_system` + `cp.charge_point`, read every raw JSON frame.
- [X] Drive it with `--remote-start-after` instead of `--autostart`.
- [X] Break `Authorize` on purpose (`idTag = BADCAFE`) and trace the abort.

- **Commit:** your own session log.

### Day 3 · The pure logic / 纯逻辑

- [X] `cp/profile.py` TODO 1 — `Recurring` profiles (daily night tariff).
- [X] TODO 2 — W ↔ A conversion. Write down your assumed nominal voltage.
- [X] TODO 3 — `minChargingRate` → `SuspendedEVSE`.
- [X] `cp/state_machine.py` TODO 1 — error-code latch.
...
- [X] Finish the `ui_button` TODO so the dashboard replaces the inject nodes.
